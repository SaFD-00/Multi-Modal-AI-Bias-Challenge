"""다중 VLM(family) SFT용 멀티모달 collator.
...
텍스트/label 패딩은 family 공통이고, **멀티모달 텐서 배칭만** family별로 다르므로
models.collate_multimodal(family, ...)에 위임한다(llava_ov=anyres patch zero-pad,
qwen 계열=pixel flatten + image_grid_thw concat).
...
from .models import collate_multimodal
...
class VLMCollator:
    ...
    family로 멀티모달 텐서 배칭 방식을 분기한다(llava_ov / qwen2_5_vl / mimo_vl).
    """

    def __init__(self, processor, family="llava_ov", img_size=224, unknown_lexicon=None, max_length=4096):
    ...
    def _encode_one(self, item):
        ...
        # 멀티모달 키(텍스트 키 제외)는 processor 원본(batch-of-1) 형태 그대로 보존.
        # family별 배칭은 __call__에서 models.collate_multimodal에 위임한다.
        mm = {k: v for k, v in enc.items() if k not in ("input_ids", "attention_mask")}
        return {
            "input_ids": input_ids,
    ...
    def __call__(self, batch):
        ...
        out.update(collate_multimodal(self.family, [e["mm"] for e in encoded]))
        return out
