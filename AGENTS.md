...
하이브리드(SB-Bench 실사 + BBQ 텍스트)로 `train.csv` 45,004행 + 실사 이미지 14,578장을 산출하고,
이를 베이스라인과 동일한 `llava-onevision-qwen2-0.5b-si-hf`로 LoRA fine-tuning한다. 상세는 `README.md` 참조.

- 데이터 코드: `src/{common,map_sbbench,augment_bbq,compose,metadata,validate}.py`
- 학습 코드: `src/train/{prompt,dataset,collator,train,merge}.py`, 설정 `configs/train_lora.yaml`
- 테스트 `tests/`(48개: 데이터 31 + 학습 17)
- 설정: `config.yaml`(데이터), 의존성 `requirements.txt`(데이터+학습 통합; torch는 cu128 휠 별도 설치, transformers==4.57.6 핀)
- Colab: `run_pipeline_colab.ipynb`(기존 `src/` 실행)
- 추론은 **범위 밖** — 베이스라인 노트북(`.claude/references/`)에 merge된 체크포인트 경로만 넣으면 동작.
...
.venv/bin/python -m pytest tests/ -q                                # 테스트 (48개)
...
> 데이터 구축은 GPU 불필요. 학습은 H100(GPU1) conda env에서 실행. `requirements.txt`가 학습 의존성까지 포함(별도 `requirements-train.txt` 없음). `data/`·`outputs/`는 `/data`로 심링크. 하이퍼파라미터는 H100 80GB 실측 기준 batch 16/accum 2(eff 32)/lr 2e-4 (batch 32는 vocab logits로 OOM).
