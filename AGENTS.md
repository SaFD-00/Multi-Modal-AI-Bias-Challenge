...
- 설정: `config.yaml`(데이터), 의존성 `requirements.txt`(데이터) / `requirements-train.txt`(학습)
...
python -m src.train.train --config configs/train_lora.yaml          # LoRA 학습 + wandb 모니터링
python -m src.train.merge --adapter outputs/llava_ov_lora --out outputs/llava_ov_merged  # 추론용 병합
```

> 데이터 구축은 GPU 불필요(`uv`로 venv). 학습은 별도 A100 환경에서 `requirements-train.txt` 설치.
