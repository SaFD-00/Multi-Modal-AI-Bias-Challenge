...
- 설정: `config.yaml`(데이터), 의존성 `requirements.txt`(데이터+학습+추론 통합; torch는 cu128 휠 별도 설치, transformers==4.57.6 핀). ⚠️ vLLM(추론)은 transformers/torch를 자체 고정해 학습 핀과 충돌 가능 → 추론은 별도 venv 권장.
