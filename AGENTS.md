...
**(1) Train 데이터 구축 파이프라인 + (2) LLaVA-OneVision 학습 스크립트(LoRA/full) 구현 완료 + (3) Full FT 1차 학습·평가 완료.**
하이브리드(SB-Bench 실사 + BBQ 텍스트 + FairFace·MMBias 외부 이미지)로 `train.csv`와 실사 이미지를
산출하고, 이를 베이스라인과 동일한 `llava-onevision-qwen2-0.5b-si-hf`로 fine-tuning한다. 상세는 `README.md` 참조.

> **Full FT 결과 (2026-06-07, RTX5090×1)**: `configs/train_full.yaml`(lr2e-5/2ep/warmup0.1/fp32/global64)로 학습 → `outputs/llava_ov_full`
...
> epoch1≈epoch2(수렴) → 추가 학습·외부데이터(VLBiasBench=RAIL++·합성, SocialCounterfactuals=race×gender·합성 등)는 효과·적합도·라이선스상 비권장, 현 모델로 확정.
...
> **RTX5090 로컬 머신(데이터 `.venv`만 존재, 학습용 conda-env 없음)** — 학습 전용 venv를 별도 구축한다(데이터 `.venv`엔 torch 없음, vLLM↔transformers 핀 충돌 회피 목적으로 분리). `.gitignore`에 `.venv-train/` 등록됨.
> ```bash
...
> .venv-train/bin/python -m src.train.launch --config configs/train_full.yaml --no-wandb   # .env(GPU_DEVICES=1)로 GPU1, batch4×accum16 fp32 global64
> ```
> WANDB 키 없으면 tensorboard 폴백 → loss는 stdout dict가 아닌 `outputs/<run>/runs/`의 이벤트에 기록(EventAccumulator로 읽음). 2 epoch ≈ 4.1h(~12.6s/optimizer-step). full FT trainable=494M(LLM 55%, 비전타워/projector freeze).
